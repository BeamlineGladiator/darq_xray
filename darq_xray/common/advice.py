...
Pure functions over a :class:`~darq_xray.common.machine.MachineProfile` and a
:class:`~darq_xray.config.models.CostEstimate`. Everything here is deterministic and
...
def human_bytes(nbytes: float) -> str:
    ...
    bar (`darq_xray/gui/main_window.py::_refresh_machine_status`) all route through it, so
...
def working_set_budget_bytes(profile, *, rss_floor_bytes: int) -> int:
    ...
    What a stage should hand to :func:`~darq_xray.common.alignment.align_volume_streamed`
...
def advise_3d(profile, shape, mode: str, requested: int = 0) -> Advice:
    ...
    *requested* is the stage's ``volume_downsample``: ``0`` (the default) means
    ...
    The factor named here comes from :func:`darq_xray.common.render3d.fit_factor_for_shape`
