...
viewer) lives in :mod:`darq_xray.common.render3d`, which reuses this module's
...
def save_layer_pngs(
    ...
    *progress* takes a **local** 0..1 fraction — wrap it with
    `darq_xray.common.progress.sub_progress` to place it in a caller's range. This is
...
def save_layer_animation(
    ...
    *progress* takes a **local** 0..1 fraction — wrap it with
    `darq_xray.common.progress.sub_progress` to place it in a caller's range. Like
