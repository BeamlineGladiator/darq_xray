...
# both render loaders use — the GUI's 3-D viewer (`darq_xray.gui.viewers._rocking_source`)
# and the rotation-video export child (`darq_xray.viewer_jobs._load_volume`) — so the
...
# than in `darq_xray/gui/` is what lets `darq_xray/` use them: the GUI depends on the core,
# never the reverse.
...
def centring_scaffold_bytes(method: str, n_values: int) -> tuple[int, int]:
    ...
    is exactly what :func:`darq_xray.common.alignment.align_volume_streamed` does,
