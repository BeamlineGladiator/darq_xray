...
A thin selection dialog over ``darq_xray.stages.slices.read_marks`` output; the
caller (profiles StageView) opens one LinePickerDialog per checked row and
appends complete jobs via ``darq_xray.gui.viewers.append_line_job``. Holds no h5 handle.
"""
