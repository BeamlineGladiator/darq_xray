...
:class:`~darq_xray.gui.widgets.param_form.ParamForm`, launches the run in a child
process via :class:`~darq_xray.runner.StageRunner`, and polls it from a
...
from darq_xray.common import roi as roi_mod
from darq_xray.common.advisory import HINT_3D_TEXTURE
from darq_xray.common.eta import EtaEstimator
from darq_xray.common.figures import FigureSpec, figures_for
from darq_xray.common.plotting import PlotStyle
from darq_xray.config.models import Experiment, StageSpec
from darq_xray.runner import Done, Failed, Log, Progress, StageRunner
...
def _roi_previews_for(stage_name: str, vals: dict):
    ...
    if stage_name == "strain":
        from darq_xray.stages import strain as m
    elif stage_name == "visualize":
        from darq_xray.stages import visualize as m
    elif stage_name == "paraview":
        from darq_xray.stages import paraview as m
    elif stage_name == "slices":
        from darq_xray.stages import slices as m
    else:
        return []
...
class StageView(QWidget):
    ...
    def _on_jobs_from_marks(self) -> None:
        ...
        if not h5 or not os.path.exists(h5):
            ...
            self._tabs.setCurrentWidget(self._log)
            return
        from darq_xray.stages import slices as _sl  # local import: lazy, Qt-free
    ...
    def _on_replot(self) -> None:
        ...
        from darq_xray.stages import mosaicity as _mo
        from darq_xray.stages import rocking as _ro
        from darq_xray.stages import strain as _st
    ...
    def _finish_ok(self, result) -> None:
        ...
        if self._vol3d is not None:
            ...
            window = self.window()
            if hasattr(window, "global_plot_style"):
                from darq_xray.common.plotting import style_to_json
