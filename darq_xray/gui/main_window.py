...
The left column holds the :class:`~darq_xray.gui.experiment_panel.ExperimentPanel`
...
side is a stacked set of :class:`~darq_xray.gui.stage_view.StageView` panels behind
an :class:`~darq_xray.gui.overview_page.OverviewPage` landing page.
"""
...
from darq_xray.common.plotting import PUBLICATION_STYLE, PlotStyle
...
class MainWindow(QMainWindow):
    ...
    def _load_plot_style() -> PlotStyle:
        ...
        from darq_xray.common.plotting import style_from_json
    ...
    def _builder_defaults(self) -> dict[str, dict]:
        ...
        Consumed by ``darq_xray.gui.widgets.panel_picker.AddPanelDialog`` (via the
    ...
    def _derive_stage_output_h5(stage: str, values: dict, chained: dict) -> str:
        ...
        ``root_folder`` (batch mode) — see ``darq_xray.stages.strain.run``/
        ``darq_xray.stages.mosaicity.run``.
    ...
    def closeEvent(self, event) -> None:  # Qt hook
        ...
        # (darq_xray/gui/widgets/busy.py) only stops those threads from being
