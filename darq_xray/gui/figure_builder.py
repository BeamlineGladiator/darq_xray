...
Qt-free recipe model (:mod:`darq_xray.compose.recipe`) is imported at module
...
from darq_xray.common.plotting import CMAP_CHOICES, SCALE_BAR_LOCS, PlotStyle, style_from_params
...
class FigureBuilderWindow(QMainWindow):
    ...
    ``{stage: {...}}`` dict :class:`~darq_xray.gui.widgets.panel_picker.AddPanelDialog`
    needs. ``style`` is the session's live :class:`~darq_xray.common.plotting.PlotStyle`;
    ...
    def _build_right_pane(self) -> QWidget:
        ...
        # end. `role="primary"` paints it in the accent (see darq_xray/gui/theme.py) — it is
    ...
    def _purge_orphaned_panels(self) -> None:
        ...
        only covers layout leaves) — see ``darq_xray.compose.render.render_recipe``.
    ...
    def _show_figure(self, figure) -> None:
        ...
        a new plain canvas here (inside a :class:`~darq_xray.gui.widgets.fit_canvas.FitFigureHost`,
        ...
        in the export), never the themed :class:`~darq_xray.gui.widgets.mpl_canvas.MplCanvas`
    ...
    def closeEvent(self, event) -> None:  # noqa: N802 — Qt override signature
        ...
        # this window's lifetime (darq_xray/gui/widgets/busy.py's wait_for_workers is
