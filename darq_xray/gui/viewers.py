...
def _viewer_headroom_bytes() -> int:
    ...
    from darq_xray.common.volumeio import display_headroom_bytes
...
def _decimation_for(dset) -> int:
    ...
    The policy itself lives in :mod:`darq_xray.common.volumeio` so the rotation-video
    export child (``darq_xray.viewer_jobs``) decimates identically — ``darq_xray/`` cannot
    import ``darq_xray/gui/``. Only the budget lookup stays here, as the monkeypatch seam.
    """
    from darq_xray.common.volumeio import display_decimation
...
def _rocking_source(aligned_path: str, dataset: str) -> Callable[[], LoadedVolume]:
    def _load() -> LoadedVolume:
        ...
        if step > 1:
            from darq_xray.common.volumeio import decimation_note
