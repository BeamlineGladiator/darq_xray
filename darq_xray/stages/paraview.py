...
:mod:`darq_xray.common.alignment`, so the PVTI is voxel-identical / co-registered),
...
# what :func:`~darq_xray.common.advice.working_set_budget_bytes` must take off the
...
STAGE = StageSpec(
    ...
    estimate="darq_xray.stages.paraview:estimate",
)
...
class _SlabReader:
    ...
    :func:`~darq_xray.common.alignment.align_volume_streamed` solves a different
...
def save_volumes_streamed(
    ...
    *,
    ...
    :class:`~darq_xray.common.alignment.StreamedAlignment`. Every provider must share
...
def estimate(params: dict) -> CostEstimate:
    ...
    **MEASURED against the real STO2 dataset at master a424b1f** (same inputs as
    ...
    to :func:`~darq_xray.common.alignment.align_volume_streamed`; ``visualize`` and
...
def _scratch_dir_for(out_dir: str, needed_bytes: int, notes: list) -> str | None:
    ...
    ``None`` is not a failure: :func:`~darq_xray.common.alignment.align_volume_streamed`
...
def _fits_in_core(providers: dict) -> bool:
    ...
    The same question :func:`~darq_xray.common.advice.plan_run` asks, in the budget's
    own working-set currency: :func:`~darq_xray.common.alignment.align_volume_streamed`
...
def _writable_providers(
    ...
    when it does not — :func:`~darq_xray.common.advice.plan_run`'s own choice, made
    per export.
...
def _unaligned_field(dset, *, roi_x, roi_y, take_abs: bool, center_method: str | None):
    ...
    :func:`~darq_xray.common.alignment.align_volume_streamed`, which always
