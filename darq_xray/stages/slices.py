...
* ``source = "stacked"`` — mosaicity/strain stacks; this stage runs the SAME
  alignment as the visualize/PVTI stages (reusing :mod:`darq_xray.common.alignment`)
...
STAGE = StageSpec(
    ...
    estimate="darq_xray.stages.slices:estimate",
)
...
def sample_planes_streamed(prep, plane_origins, u_hat, v_hat, half_u, half_v, du, dv):
    ...
    :func:`~darq_xray.common.volumeio.iter_with_context` supplies. A sample assigned to
...
class _Float64Dataset:
    ...
    :func:`~darq_xray.common.alignment.align_volume_streamed` runs ``abs`` and the
...
def _materialise(blocks, shape, dtype):
    """This stage's name for :func:`~darq_xray.common.alignment.materialise_blocks`.
...
def _aligned_block_budget(dset, budget_bytes: int) -> int:
    ...
    :func:`~darq_xray.common.alignment.align_volume_streamed` prices at
...
def _unaligned_pad(samy, n_layers: int, scale_x: float, samy_direction: int):
    ...
    Reproduces :func:`~darq_xray.common.alignment.apply_samy_shifts_to_volume`'s own
...
def _unaligned_blocks(
    ...
    **whole** volume and passed in explicitly, which is the same trick — and the
    same reason — as :func:`~darq_xray.common.alignment.align_volume_streamed`'s. A
...
def build_slice_figure(
    ...
    renderer. When a :class:`~darq_xray.common.plotting.PlotStyle` is supplied its
...
def estimate(params: dict) -> CostEstimate:
    ...
    * ``interpolate_to_uniform_z`` resamples onto
    ...
    along Z is exactly what :func:`~darq_xray.common.alignment.align_volume_streamed`
...
def _run_budget_bytes(p: dict, out_dir: str | None = None) -> int:
    ...
    :func:`~darq_xray.common.advice.working_set_budget_bytes` with this stage's own
...
def run(params: dict, progress: ProgressFn | None = None) -> SlicesResult:
    ...
    fh.attrs["created_by"] = "darq_xray.stages.slices"
            ...
            with contextlib.ExitStack() as open_files:
                ...
                for si, sl in enumerate(slices):
                    ...
                    for pi, s2d in enumerate(planes):
                        ...
                        # `darq_xray/common/render.py` documents the same rule for
