...
1. aligns the stacked volume — ROI -> samy sub-pixel X-shift -> uniform-Z
   interpolation — reusing :mod:`darq_xray.common.alignment` (golden-tested,
...
# :func:`~darq_xray.common.advice.working_set_budget_bytes` must take off the
...
STAGE = StageSpec(
    ...
    estimate="darq_xray.stages.visualize:estimate",
)
...
def _shifted(blocks, offset):
    ...
    :func:`~darq_xray.common.alignment.align_volume_streamed`'s own, for the same
...
def _parse_opacity(text, param_name: str) -> float | None:
    ...
    A :class:`~darq_xray.common.errors.StageUserError` rather than a bare
...
def _opacity_for(name: str, p: dict) -> tuple[float, str]:
    ...
    :class:`~darq_xray.common.render3d.Scene3D` — cannot disagree about it, and so a
...
def _clim_overrides(p: dict) -> dict[str, tuple[float | None, float | None]]:
    ...
    Every rejection is a :class:`~darq_xray.common.errors.StageUserError` with a
...
class _LayerSource:
    ...
    :func:`~darq_xray.common.render.save_layer_pngs` and
    :func:`~darq_xray.common.render.save_layer_animation` consume: they use ``.shape``
    ...
    def whole(self) -> np.ndarray:
        ...
        :func:`~darq_xray.common.alignment.materialise_blocks`, not a fourth copy of
...
def _fits_in_core(streamed) -> bool:
    ...
    The same question :func:`~darq_xray.common.alignment.align_volume_streamed` asks
...
def _run_budget_bytes(p: dict, out_dir: str | None = None) -> int:
    ...
    ``plot_style`` snapshot ``darq_xray/gui/stage_view.py`` injects at run time. Tests use
    ...
    :func:`~darq_xray.common.alignment.align_volume_streamed` prices its working set
    ...
    goes through :func:`~darq_xray.common.advice.working_set_budget_bytes` with this
...
def aligned_field(params: dict, name: str):
    ...
    **The return value cannot stream**, and that is a property of the consumer,
    not of this function: `darq_xray/gui/viewers.py::_visualize_load` hands the array to
    ...
    reads the field as a :class:`~darq_xray.common.alignment.StreamedAlignment`,
