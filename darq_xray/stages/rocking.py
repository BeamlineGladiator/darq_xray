...
4. aligns both volumes to the SAME mosa references as the other stages
   ...
   mosa_samz[0]`` for the Z origin) via :mod:`darq_xray.common.alignment`, so they
...
5. saves both aligned volumes + alignment metadata to one HDF5 file (consumed
   ...
   animation, and a 3-D top-view via :mod:`darq_xray.common.render`.
...
STAGE = StageSpec(
    ...
    estimate="darq_xray.stages.rocking:estimate",
)
...
def _replot_default_clim(
    ...
    **Two rungs, one answer.** When the whole volume fits the working-set budget
    ...
    :func:`~darq_xray.common.volumeio.stream_quantile`, which returns what
