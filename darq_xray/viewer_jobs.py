"""Qt-free jobs the GUI runs in a child process (via darq_xray.runner.StageRunner).

Sits ABOVE darq_xray.common and darq_xray.stages in the layering (it may import both);
darq_xray/gui/ builds the JSON-able params, the runner resolves
"darq_xray.viewer_jobs:rotation_video_job" in the child.
"""
...
def _load_volume(loader: dict):
    ...
    :mod:`darq_xray.common.volumeio`'s display policy, so the exported video is
