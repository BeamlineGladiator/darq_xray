...
| **Run / Cancel + progress** | Runs the stage in a **separate process**; the bar and step text track progress; **Cancel** truly kills it. Before launching, input paths are checked on disk — a missing one blocks the run and focuses the offending field. Once a run/batch is more than 5 % done and has been going for more than 2 seconds, the progress text may also show a `~… left` estimate; the estimate starts fresh every time you click **Run**, so it never carries a stale reading over from a previous run. It is measured from the run's **recent** pace rather than its average, counts down while a stage is busy without reporting, and **goes quiet rather than showing `~1 s left`** once a run has outlasted its own estimate — no number means it genuinely does not know. Every stage now reports **continuously** rather than at phase boundaries — the bar advances per layer, per volume, per `.vti` piece, per job — so it no longer stands still through long stretches of a run (`visualize` used to sit at 60 % for its entire strain half, and `paraview` reported four times for a whole export). It is still an estimate: it re-bases on the run's current pace, so it can step up or down when a stage moves from one kind of work to another. |
