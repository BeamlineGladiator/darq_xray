...
- `load_mosa_datasets` / `load_strain_volume`, `_align(...)` (reuses [[#`alignment.py`]]), `_process_dataset(..., style=None)` (threads the run's style into the layer PNGs/animation and builds the one `render3d.Scene3D` — volume, `(sx, sy, scale_z)` spacing, colormap, run clim, `volume_opacity` (scaling the volume transfer function, see `_volume_opacity`), `mode="volume"` — shared by the top view and the rotation video, so the two always show the same render).
