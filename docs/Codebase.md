...
| `sum_dataset_bytes(path) -> (total_bytes, largest_shape, largest_itemsize)` | Total in-memory size of every dataset in *path*, from shapes alone (`f.visititems`, reads no data). `(0, None, 0)` for anything unreadable. Used by the `slices`/`rocking`/`matched`/`paraview`/`visualize` `estimate()` functions. |
...
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. Globs `raw_root` with `rocking_pattern`, sizes **one** matched scan folder's `detector_path` dataset (default `1.1/measurement/pco_ff`, via `resolve_input_file` — never opens data), and multiplies by the folder count. `chunkable=True`, `peak_bytes == input_bytes` — modelled on the bare `dataset[:]` read the cold-replot path (`_replot_default_clim`) uses, with no dtype-conversion overhead. Never raises — an unresolved glob or missing dataset returns `CostEstimate(0, 0, None, True, note=...)`.
...
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. Sums every dataset in `mosa_volume_file` + `strain_volume_file` via `sum_dataset_bytes` (whole-file, no per-quantity filtering). `chunkable=True`, `peak_bytes == input_bytes` — `load_mosa_datasets`/`load_strain_volume` load without a dtype conversion. Never raises — no readable volume files returns `CostEstimate(0, 0, None, True, note=...)`.
...
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. Sums `sum_dataset_bytes` over the four volume-file params (`mosa_volume_file`, `strain_volume_file`, `aligned_rocking_file`, `aligned_mosa_file` — whole file per param, not per selected dataset). `peak_bytes = input + 2 * n * 8` — `prepare_volume` holds each volume as `[:].astype(np.float64)` (source + float64 copy) then a further float64 copy from alignment (`apply_samy_shifts_to_volume`/`interpolate_to_uniform_z`). `chunkable=False` — alignment is a whole-volume operation. Never raises — no readable volumes returns `CostEstimate(0, 0, None, False, note=...)`.
...
- `run` / `_main`.
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. Globs `raw_root` with `rocking_pattern`, sizes **one** matched scan folder's `pco_ff_path` dataset (default `1.1/measurement/pco_ff`, via `resolve_input_file`) and multiplies by the folder count. `peak_bytes = input + n * 8 + frame_elems * 8` — `load_pco_ff_frame` reads `ds[:].astype(np.float64)` (source + float64 copy) then `np.nanmedian(stack, axis=0)` builds one more float64 frame. `chunkable=False` on every branch — an exact median needs the whole stack. Never raises — an unresolved glob or missing dataset returns `CostEstimate(0, 0, None, False, note=...)`.
