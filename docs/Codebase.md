...
- `Row` / `Col` — layout containers (nest freely); each supports a pinned
  ...
  `render._apply_shared_colorbars`' provisional bar span. `group_label` is tri-state: `None` = not a group, `"auto"` = an
