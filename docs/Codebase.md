...
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. Resolves the layer work list via `resolve_layer_work`, opens **one** layer's `maps.h5`, and reads only `.shape`/`.dtype` for each of the four dataset paths present (chi/mu × CoM/FWHM) — never data. `shape = (n_layers, H, W)`; `peak_bytes = (n_present + 1) * n_layers * H * W * itemsize` — `run()` holds every layer of all present datasets in `collected` simultaneously, then `np.stack` adds one more volume, so this is one of the heaviest stages, not the layer-streaming one the `_streamed_clim` docstring might suggest. Never raises — an unresolved work list or no matching datasets returns `CostEstimate(0, 0, None, True, note=...)`.
