...
| Module | What it does |
|---|---|
| `app.py` | Entry point `main()` (`python3 -m gui.app`). Sets `QT_API=pyside6`, then **defers** Qt imports so the spawn-reimported worker child stays Qt-free. Reads the saved theme from `QSettings("dfxm", "pipeline")` key `theme` at startup and applies it via `ThemeController.set_mode` before the window is shown. After `app.exec()` returns, calls `gui.widgets.busy.wait_for_workers()` defensively (fix wave, review F1) before returning — `MainWindow.closeEvent` already joins every pinned worker, but `app.exec()` can also return via a path that skips `closeEvent` (e.g. some other quit trigger), and a still-pinned running `QThread` left at interpreter teardown aborts the process; a no-op when `closeEvent` already emptied the live-worker registry. |
