...
- `_layer_clim(dataset)` — stream over all Z layers of an open HDF5 dataset to compute `(vmin, vmax)` from finite values; returns `(0.0, 1.0)` when all values are non-finite.
