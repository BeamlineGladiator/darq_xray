...
- `build_trace_figure(fld, geom, *, aspect_wh, width_in, linewidth, color, font_scale, style=None)` — build and return a standalone line-profile `Figure` for a single field (distance vs value_mean + optional std band). `aspect_wh=(w,h)` pins the **plot box** (data rectangle) to exactly `w:h` via `ax.set_box_aspect(h/w)` — the plotted area keeps the ratio regardless of label/title/font margins; the figure canvas is `(width_in, width_in*h/w)` so it roughly matches the box (tight-crop on save then hugs box+labels). When the style carries a fixed physical scale, the box WIDTH is `geom["L"]/scale` cm instead and `width_in` is ignored (same rule as `figure_width` for maps) — the scale is the TRACE-effective one from `trace_fixed_scale(style)` (`trace_scale_um_per_cm` when set, else `scale_um_per_cm`), passed to `fixed_scale_box(style, L, L·h/w, scale=...)` (30-in clamp preserved on the trace aspect) with `fit_axes_to_box` run last inside the builder, so run/replot/export traces all print the distance axis at the same chosen µm-per-cm. All trace text is multiplied by `font_scale` (its own scale, independent of the map `style.font_scale`); the curve/band use `color` (blank/`None` → `"C0"`). A supplied `style` additionally applies `show_title` (omit the `kind | field | source` header) and `title_scale` (multiplies the title font alone); `style=None` keeps the legacy always-on title. `kind="plot"`, no colorbar. Does NOT call `savefig`.
- `_save_traces(out_dir, stem, fields, geom, *, aspect, width_in, linewidth, color, font_scale, dpi, style=None)` — build+save one `{stem}__trace__{vid}.png` per field; returns the paths. The PNG is always saved with `bbox_inches="tight"` (a tight crop trims the canvas without rescaling a fixed-scale-fitted box). The former `trace_file_aspect` param/`file_aspect` kwarg (pin the outer PNG ratio) was removed 2026-07-20 — stale persisted form values carrying it ride along ignored.
