...
| `_volume_opacity(scene)` | The named `opacity_mapping` built explicitly as a 256-step alpha curve (`pv.opacity_transfer_function`) and scaled by `scene.opacity`, for `add_volume(opacity=…)`. Handing `add_volume` the mapping *name* always yields the full 0–255 ramp, which made the scalar opacity a silent no-op in volume mode (the stages' `volume_opacity` did nothing); scaling the curve makes one opacity knob mean the same thing in every render mode. |
