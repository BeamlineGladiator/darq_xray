...
| `volume3d.py` | `Volume3DPanel`: a volume dropdown + **Render 3-D** button over a (lazy) `PvCanvas`. `set_sources(dict[str, VolumeSourceSpec])` installs the specs — nothing loads until the button is clicked; `_on_render` then calls `spec.load()` and renders the returned `LoadedVolume` (`.volume`/`.spacing`/`.cmap`/`.clim`). |
