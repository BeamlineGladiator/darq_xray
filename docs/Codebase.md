...
- `ComposeStyle` — composer-level look knobs: `label_template`, `label_font_scale`,
  ...
  `style.font_scale`); `validate_recipe` rejects a non-positive
  `trace_linewidth`/`trace_font_scale` (`StageUserError` + hint). Resolved by
  `adapters.resolve_trace_opts` (below). All of these are additive fields:
