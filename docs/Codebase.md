...
| `ReplotGroup` | Dataclass with `key: str`, `label: str`, `item_labels: list[str]`. Represents one selectable group in a replot catalog (a dataset/product with N layers); used by the mosaicity/rocking cold-replot paths. |
...
- `render_replot(h5_path, selections, style, clim, out_dir, roi=None, params=None) -> list[str]` — cold-replot selected strain layers: `selections` is `list[("strain", item_idxs | None)]` (`None` = all layers). Delegates each layer to `_rebuild_strain_map` (with optional `roi` and `clim` override), writes PNGs under `{out_dir}/strain/` (e.g. `strain/a_strain.png`), and returns the list of written paths. Out-of-range indices and layers where `_rebuild_strain_map` returns `None` (empty ROI crop) are silently skipped.
- `run` / `_main`.
...
- `render_replot(h5_path, selections, style, clim, out_dir, roi=None, params=None) -> list[str]` — cold-replot selected layers: `selections` is `list[(dataset_key, item_idxs | None)]` (`None` = all layers). Reads per-dataset clim via `_streamed_clim`, delegates each layer to `render_volume_layer` (with optional `roi` crop and `clim` override), writes PNGs under `{out_dir}/{stem}/` (e.g. `chi_com/chi_com_layer_0000.png`), and returns the list of written paths. Layers where `render_volume_layer` returns `None` (empty ROI crop) are silently skipped.
...
- `render_replot(h5_path, selections, style, clim, out_dir, roi=None, params=None) -> list[str]` — cold-replot selected layers: `selections` is `list[(dataset_key, item_idxs | None)]` (`None` = all layers). Pixel scales fall back to `STAGE.defaults()` when absent from `params`. When `clim` is unset, the per-dataset default limits come from `_replot_default_clim` (percentile-based, matching the run); source-aware titles/cbar (`_sum_title`/`_spec_title` + `normalize_sum` tag) are rebuilt from `params`, degrading to the generic `_DATASET_DISPLAY` labels when params are absent. Delegates each layer to `render_volume_layer` (with optional `roi` crop and `clim` override), writes PNGs under `{out_dir}/{key}/` (e.g. `sum_intensity/sum_intensity_layer_0000.png`), and returns the list of written paths. Layers where `render_volume_layer` returns `None` (empty ROI crop) are silently skipped.
...
- `ReplotEntry` — dataclass: `volume_id: str`, `slice_name: str`, `n_planes: int`, `offsets_um: list[float]`.
- `replot_catalog(h5_path: str) -> list[ReplotEntry]` — enumerate every `(volume_id, slice_name)` present in an `oblique_slices.h5`, with plane count and offset list. Skips any sub-group that doesn't contain a `slices` dataset.
- `render_replot(h5_path, selections, style, clim, out_dir, roi=None, *, dpi=150) -> list[str]` — rebuild + save selected planes (appearance-only; no resampling). `selections` is a list of `(vid, slice_name, plane_idxs)` where `plane_idxs=None` means all planes; out-of-range indices and unknown `(vid, slice)` pairs are silently skipped. `roi=(r0, r1, c0, c1)` is an optional pixel-index crop passed through to `_rebuild_plane_figure`; planes whose clamped crop is empty are silently skipped (never raise). PNGs are written under `{out_dir}/{slice_name}/` mirroring the run layout (`{vid}.png` for single-plane slices, `{vid}__p{k:03d}_{off:+08.2f}um.png` for sweeps). Returns the list of written paths. `clim` is an optional `(vmin, vmax)` override passed through to `_rebuild_plane_figure`.
...
| `slice_replot.py` | `SliceReplotDialog`: pick volumes/slices/planes from an `oblique_slices.h5` and re-render PNGs with the current publication style + optional colour-limit override + optional ROI crop. Reads the HDF5 directly from disk (works from a cold start with no prior run). Tree is checkable per-plane (default unchecked); `select_all()` checks every leaf. `_clim()` parses the vmin/vmax boxes (both blank → `None`). `_roi()` parses the four ROI boxes (`_r0/_r1/_c0/_c1`): all four filled → `(r0,r1,c0,c1)` tuple of ints; any blank → `None` (partial ROI is ignored, matching the four-box contract of `ReplotDialog`). `render_selection(out_dir)` delegates to `slices.render_replot(..., roi=self._roi())` and returns written paths. Wired into `StageView` via `_on_replot` (the **Replot…** button, always enabled, on the slices stage panel); the slot snapshots the session `PlotStyle` via `window.global_plot_style()` and passes a defensive copy to the dialog. Output lands in a timestamped `replots/<stamp>/` subfolder of the slices output dir (absolute path). |
| `replot_dialog.py` | `ReplotDialog(h5_default, catalog_fn, render_fn, style=None, out_default="", parent=None)`: generic 2-level checkable-tree replot dialog serving strain/mosaicity/rocking uniformly. `catalog_fn(h5_path) -> list[ReplotGroup]` populates a group → layer tree (tristate parent, leaf children); `render_fn(h5_path, selections, style, clim, roi, out_dir) -> list[str]` is called on **Render**. A file field (editable + Browse + Load) reloads the catalog cold — keeps the dialog working when opened without a prior stage run. `select_all()` / `_deselect_all()` bulk-check/uncheck top-level groups. `_selections() -> list[(group_key, item_idxs | None)]` reads the tree: `None` means whole group (all layers checked). `render_selection(out_dir) -> list[str]` calls `render_fn` and stores `.written`. **clim contract:** both blank → `None`; either filled → `(vmin|None, vmax|None)`. **ROI contract:** all four boxes (`r0,r1,c0,c1`) must be filled together → `(r0,r1,c0,c1)` tuple; any blank → `None` (partial ROI never forwarded). Imported on demand inside `_on_replot` by the wiring task (Task 8). |
