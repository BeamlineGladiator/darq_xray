...
- `append(dataset_path, layer)` — add one 2-D `layer` to `dataset_path`, creating the dataset on first use as `shape=(0, *layer.shape)`, `maxshape=(None, …)`, `chunks=(1, *layer.shape)` with the writer's compression (`compression_opts=4` for gzip), then `resize`-and-write one more row. `dataset_path` may be a nested path (`"chi/Center of mass"`); intermediate groups are created implicitly by h5py, so a group only exists when it actually holds a dataset. Raises `ValueError("<path>: maps have differing shapes {…}; fix ROI")` when a layer's shape disagrees with the rows already written. Per-dataset layer counts may legitimately differ (a mosaicity layer can be missing one of the four maps).
- `shape(dataset_path) -> tuple[int, ...]` — the dataset's current `(Z, Y, X)`; `datasets() -> list[str]` — the appended paths, sorted; `set_attrs(**attrs)` — file-level attributes.
- `close()` — flush, close, and `os.replace` the working `<path>.part` onto `<path>`. `abort()` — close and unlink the part file, swallowing errors so it never masks the caller's exception. Both are **no-ops once the file has been committed or aborted** (`_closed`), which is what makes the stages' "no layers produced" path legal: they call `abort()` and `return` from inside the `with`, leaving `__exit__` to call `close()` on a part file that is already gone. A *first* `close()` still propagates whatever failed.
- Context manager: commit on a clean exit, abort on an exception — so a failure mid-run leaves neither the output nor a stray `.part`, the same all-or-nothing behaviour the write-at-the-end version had for free.
...
| `block_nansum(dset, *, budget_bytes)` | Sum of the *finite* values, built on `block_reduce` + `neumaier_sum`; bit-identical for any `budget_bytes`. Ignores all non-finite values — NaN **and** ±inf (unlike `np.nansum`, which propagates inf; inf would poison the compensation term). On all-finite data it differs from `np.nansum` by up to ~1 ulp — numpy's pairwise reduction order is itself budget-dependent and cannot give the cross-budget guarantee. |
...
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. Sums every dataset in `mosa_volume_file` + `strain_volume_file` via `sum_dataset_bytes` (whole-file, no per-quantity filtering) for `input_bytes`. Unlike `paraview`, `run()` handles the mosaicity and strain sections **inline in the same function scope** rather than via separate helper calls — the mosaicity `datasets` dict (raw, native dtype, every field) is never deleted or reassigned, so it stays alive through the strain section too, and the two files' input bytes **add** (not max, unlike `paraview`). `peak_bytes = total_input + 3 * largest_elems * 8` — each `_align` call (`apply_roi_3d` -> `apply_samy_shifts_to_volume` -> `interpolate_to_uniform_z`, which upcasts to float64) leaves up to three float64-sized temporaries of the largest field alive at once. `chunkable=True`. Never raises — no readable volume files returns `CostEstimate(0, 0, None, True, note=...)`.
...
- `roi_previews(params) -> list[tuple[str, Callable]]` — ROI-picker hook for the `roi_x`/`roi_y` params (tagged `roi_group="crop"`, `roi_axis="x"`/`"y"`). Delegates to `stacked_volume_previews(params)` in `dfxm.common.figures`. Returns `[]` when the volume files cannot be read.
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. Sums every dataset in `mosa_volume_file` + `strain_volume_file` via `sum_dataset_bytes` for `input_bytes`. `run()` processes the two files **sequentially** through separate helper calls (`_process_mosaicity` then `_process_strain`) — each helper's locals, including the raw `datasets` dict it loads, die when it returns, so the peak is the **max** over the (at most two) files, not their sum. Within one file, the helper keeps the raw dataset(s) (native dtype, `file_total` bytes) alive while building an aligned float64 copy of every field (`file_elems * 8`), and `save_volumes_as_pvti` then builds a further `np.where`-cleaned float64 copy of every field plus a boolean `valid_mask` before downcasting to `SAVE_DTYPE` per piece at write time (`file_elems * 8 + largest_elems * 8`). Per-file peak: `file_total + 2 * file_elems * 8 + largest_elems * 8`; `peak_bytes` is the max of that across the selected files. `chunkable=True`. Never raises — no readable volume files returns `CostEstimate(0, 0, None, True, note=...)`.
...
- `estimate(params) -> CostEstimate` — shape-only cost prediction wired into `STAGE.estimate`. `input_bytes` sums `sum_dataset_bytes` over the four volume-file params (whole file per param, not per selected dataset — kept for continuity with the other estimators). `peak_bytes` models `run()`'s real loop: `prepare_volume` is called one selected dataset at a time and its result (`prep`) is a plain local that gets *rebound* each iteration, so the previous volume's prepared array stays alive while the next is built — the peak pairs the current volume's own load peak with the largest OTHER selected volume's footprint, not the sum of all of them. Per dataset `v` (resolved via `iter_dataset_sizes` against `_standard_volumes`' `include_*`-toggled selection): a **stacked** source (`mosa_volume_file`/`strain_volume_file`) holds the native read plus three float64-sized copies (`[:].astype(np.float64)`, the samy-shifted canvas, `interpolate_to_uniform_z`'s output) — `load_peak_v = elems_v * itemsize_v + 3 * elems_v * 8`; an **aligned** source (`aligned_rocking_file`/`aligned_mosa_file`) is already co-registered, so only the one float64 read persists — `load_peak_v = elems_v * itemsize_v + 1 * elems_v * 8`. `peak_bytes = max_v(load_peak_v + max_{w≠v}(elems_w * 8))`. Falls back to the coarser `input_bytes` figure if no selected dataset resolves. `chunkable=False` — alignment is a whole-volume operation. Never raises — no readable volumes returns `CostEstimate(0, 0, None, False, note=...)`.
