...
- `_write_image_video(get_frame, n_frames, base_path, fmt, fps=15)` — GL-free frame→movie assembler: `get_frame(i)` returns an `(H, W, 3)` uint8 RGB frame, drawn through a borderless `Figure`/`imshow` and saved with the same MP4 (ffmpeg) → GIF fallback semantics as `save_layer_animation`; returns the written path.
- `save_rotation_video(volume, scale_z, sx, sy, vmin, vmax, cmap, opacity, base_path, fmt, *, n_frames=120, fps=15)` — 360° orbit movie of the same pyvista volume render as `save_top_view` (**lazy** import, same `_pyvista_grid`/`add_mesh` styling): starts from the isometric view, steps the camera azimuth by `360/n_frames` per frame, grabs frames one at a time (no full frame stack in memory), and assembles via `_write_image_video`. Returns the written path or `None` when the NaN-thresholded grid is empty.
