...
- `_save_animation(anim, base_path, fmt, fps, dpi)` — the single MP4 (ffmpeg) → GIF fallback policy shared by `save_layer_animation` and `_write_image_video`: `fmt` is `mp4`/`gif`/`both`; a failed MP4 write removes the partial `.mp4` before falling back to GIF; returns the written path.
- `_pyvista_grid(data, spacing)` / `save_top_view(...)` — 3-D top-view render (**lazy** `pyvista` import; NaN voxels thresholded out).
- `_volume_plotter(volume, scale_z, sx, sy, vmin, vmax, cmap, opacity) -> Plotter | None` — the single off-screen plotter + `add_mesh` styling shared by `save_top_view` and `save_rotation_video` (so the video is guaranteed to match the top-view look); `None` when the NaN-thresholded grid is empty (**lazy** `pyvista` import).
- `_write_image_video(get_frame, n_frames, base_path, fmt, fps=15)` — GL-free frame→movie assembler: `get_frame(i)` returns an `(H, W, 3)` uint8 RGB frame, drawn through a borderless `Figure`/`imshow` and saved via `_save_animation`. `get_frame` must be idempotent in `i` — the sequence is replayed in full for `fmt="both"` and for the MP4→GIF fallback. Returns the written path.
- `save_rotation_video(volume, scale_z, sx, sy, vmin, vmax, cmap, opacity, base_path, fmt, *, n_frames=120, fps=15)` — 360° orbit movie of the same pyvista volume render as `save_top_view` (via `_volume_plotter`): starts from the isometric view and renders each frame at the *absolute* azimuth `i·360/n_frames` from a stored start pose (idempotent — replays render the identical orbit), grabbing frames one at a time (no full frame stack in memory) and assembling via `_write_image_video`. Returns the written path or `None` when the grid is empty.
...
- `run` → per-layer PNGs, animation, 3-D top-view, and (when `save_rotation` is set — default off) a rotating 3-D orbit video per dataset (`<name>_rotation.mp4`/`.gif` via `render.save_rotation_video`; reuses `volume_opacity` + `output_format`; any pyvista/GL failure becomes a `notes` entry, mirroring the top view, and an all-NaN volume gets a "no finite voxels" note instead of a silent skip); colormaps and figure styling come from the injected `plot_style` (via `style_from_params`/`resolve_cmap`).
