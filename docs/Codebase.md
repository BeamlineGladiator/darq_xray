...
| `slice_replot.py` | `SliceReplotDialog`: pick volumes/slices/planes from an `oblique_slices.h5` and re-render PNGs with the current publication style + optional colour-limit override. Reads the HDF5 directly from disk (works from a cold start with no prior run). Tree is checkable per-plane (default unchecked); `select_all()` checks every leaf. `render_selection(out_dir)` delegates to `slices.render_replot` and returns written paths. Wired into `StageView` via `_on_replot` (the **Replot…** button, always enabled, on the slices stage panel); the slot snapshots the session `PlotStyle` via `window.global_plot_style()` and passes a defensive copy to the dialog. Output lands in a timestamped `replots/<stamp>/` subfolder of the slices output dir (absolute path). |
