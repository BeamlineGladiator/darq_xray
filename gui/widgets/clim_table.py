...
:class:`~gui.widgets.jobs_summary.JobsSummaryEditor`'s contract — ``text()`` /
...
The dialog itself hosts the **existing** :class:`~gui.widgets.clim_section.ClimGroupSection`,
...
def _volume_rows() -> list[tuple[str, str]]:
    ...
    run will ignore. The import is local: ``gui`` may depend on ``dfxm``, and
    keeping it out of module scope keeps this widget cheap to import.
    """
    from dfxm.stages.visualize import _VOLUME_KEYS
